...
# run Model 1
tT_CERAD = run_model1("CERAD", mDataG, C, E, annot, "MSBBSTG", "microglia")  # CERAD (sample n = 240)
tT_Braak = run_model1("Braak", mDataG, B, E, annot, "MSBBSTG", "microglia") # Braak (sample b = 229)

nrow(tT_CERAD[E4vsE3_P.Value < 0.05]) ## 1 E4-E3
nrow(tT_Braak[E4vsE3_P.Value < 0.05]) ## 2 E4-E3

intersect(tT_Braak[E4vsE3_P.Value < 0.05]$GeneSymbol,tT_CERAD[E4vsE3_P.Value < 0.05]$GeneSymbol) # 0 in common

length(tT_CERAD[E4vsE3_P.Value < 0.05 & E4vsE3_logFC > 0,]$GeneSymbol) # 1 up in E4-E3
length(tT_CERAD[E4vsE3_P.Value < 0.05 & E4vsE3_logFC < 0,]$GeneSymbol) # 0 down in E4-E3
length(tT_Braak[E4vsE3_P.Value < 0.05 & E4vsE3_logFC > 0,]$GeneSymbol) # 2 up in E4-E3
length(tT_Braak[E4vsE3_P.Value < 0.05 & E4vsE3_logFC < 0,]$GeneSymbol) # 0 down in E4-E3

# Table S4
TableS4 = fread("TableS4.csv")
TableS4[tT_CERAD, on="EnsemblID",  c("MSBB-STG_E4vsE3_logFC", "MSBB-STG_E4vsE3_P.Value", "MSBB-STG_E2vsE3_logFC", "MSBB-STG_E2vsE3_P.Value") := .(i.E4vsE3_logFC, i.E4vsE3_P.Value, i.E2vsE3_logFC, i.E2vsE3_P.Value)]
...
nrow(tT1.sfE[tT1.sfE$P.Value < 0.05, ])  # E4Num: 17
nrow(tT2.sfE[tT2.sfE$P.Value < 0.05, ])  # E2Num: 33

E4Coeff = tT_CERAD_ENum$coefficients[,2]
E2Coeff = tT_CERAD_ENum$coefficients[,3]
all(names(E4Coeff) == tT1.sfE$EnsemblID)

# 0 in common (with E4 coeff > 0)
...
E4.Genes.1 = tT1.sfE[tT1.sfE$P.Value < 0.05 & E4Coeff > 0, "GeneSymbol"] # 17
E2.Genes.2 = tT2.sfE[tT2.sfE$P.Value < 0.05 & E2Coeff < 0, "GeneSymbol"] # 21
intersect(E4.Genes.1, E2.Genes.2) # 0

tT1.sfE$coefficients = E4Coeff
tT2.sfE$coefficients = E2Coeff
fwrite(tT1.sfE, "../Data/limmaResult/MSBB-STG-model1.5-E4Num.csv")
fwrite(tT2.sfE, "../Data/limmaResult/MSBB-STG-model1.5-E2Num.csv")

# Table S5
setDT(tT1.sfE)
setDT(tT2.sfE)
TableS5 = fread("TableS5.csv")
TableS5 = TableS5[tT1.sfE, on = .(EnsemblID, GeneSymbol), c("MSBB-STG_e4_dosage_logFC", "MSBB-STG_e4_dosage_P.Value", "MSBB-STG_e4_dosage_coefficients") := .(i.logFC, i.P.Value, i.coefficients)]
TableS5 = TableS5[tT2.sfE, on = .(EnsemblID, GeneSymbol), c("MSBB-STG_e2_dosage_logFC", "MSBB-STG_e2_dosage_P.Value", "MSBB-STG_e2_dosage_coefficients") := .(i.logFC, i.P.Value, i.coefficients)]
...
nrow(tT2_CERAD[E4vsE3_P.Value < 0.05]) # 33 E4-E3
nrow(tT2_Braak[E4vsE3_P.Value < 0.05]) # 188 E4-E3

length(intersect(tT2_Braak[E4vsE3_P.Value < 0.05]$GeneSymbol,tT2_CERAD[E4vsE3_P.Value < 0.05]$GeneSymbol)) # 22 in common

length(tT2_CERAD[E4vsE3_P.Value < 0.05 & E4vsE3_logFC > 0,]$GeneSymbol) # 33 up in E4-E3
length(tT2_CERAD[E4vsE3_P.Value < 0.05 & E4vsE3_logFC < 0,]$GeneSymbol) # 0 down in E4-E3
length(tT2_Braak[E4vsE3_P.Value < 0.05 & E4vsE3_logFC > 0,]$GeneSymbol) # 186 up in E4-E3
length(tT2_Braak[E4vsE3_P.Value < 0.05 & E4vsE3_logFC < 0,]$GeneSymbol) # 2 down in E4-E3


# add to Table S2
TableS2 = fread("TableS2.csv")
TableS2[tT2_CERAD, on="EnsemblID",  c("MSBB.STG_E4vsE3_logFC", "MSBB.STG_E4vsE3_P.Value", "MSBB.STG_E2vsE3_logFC", "MSBB.STG_E2vsE3_P.Value") := .(i.E4vsE3_logFC, i.E4vsE3_P.Value, i.E2vsE3_logFC, i.E2vsE3_P.Value)]
fwrite(TableS2, "TableS2.csv")


# add to Table S3
TableS3 = fread("TableS3.csv")
TableS3[tT_CERAD, on="EnsemblID",  c("MSBB.STG_E4vsE3_logFC", "MSBB.STG_E4vsE3_P.Value", "MSBB.STG_E2vsE3_logFC", "MSBB.STG_E2vsE3_P.Value") := .(i.E4vsE3_logFC, i.E4vsE3_P.Value, i.E2vsE3_logFC, i.E2vsE3_P.Value)]
...
nrow(tT3_CERAD[E4vsE3_P.Value < 0.05,]) # 8
nrow(tT3_Braak[E4vsE3_P.Value < 0.05,]) # 6

intersect(tT3_Braak[E4vsE3_P.Value < 0.05,]$GeneSymbol,tT3_CERAD[E4vsE3_P.Value < 0.05,]$GeneSymbol) # 2 in common

length(tT3_CERAD[E4vsE3_P.Value < 0.05 & E4vsE3_logFC > 0,]$GeneSymbol) # 7 up in E4-E3
length(tT3_CERAD[E4vsE3_P.Value < 0.05 & E4vsE3_logFC < 0,]$GeneSymbol) # 1 down in E4-E3
length(tT3_Braak[E4vsE3_P.Value < 0.05 & E4vsE3_logFC > 0,]$GeneSymbol) # 3 up in E4-E3
length(tT3_Braak[E4vsE3_P.Value < 0.05 & E4vsE3_logFC < 0,]$GeneSymbol) # 3 down in E4-E3
```
...
TableS2.2 = do.call(cbind, my.dat)
colnames(TableS2.2) = paste0("MSBB.STG_E", colnames(TableS2.2))
colnames(TableS2.2) = paste0(colnames(TableS2.2), "AveZ")
TableS2.2 = as.data.frame(TableS2.2)
TableS2.2$GeneSymbol = rownames(TableS2.2)
setDT(TableS2.2)
TableS2 = fread("TableS2.csv")
TableS2 = TableS2.2[TableS2, on = .(GeneSymbol)]
TableS2 = TableS2[order(GeneSymbol), ]
TableS2 = TableS2[, .(EnsemblID, GeneSymbol, Description, ROSMAP_E2AveZ, ROSMAP_E3AveZ, ROSMAP_E4AveZ, MSBB.STG_E2AveZ, MSBB.STG_E3AveZ, MSBB.STG_E4AveZ)]
fwrite(TableS2, "TableS2.csv")
...
# violin plot
figs1.stg.1 = ggplot(my.df, aes(x=APOE.num, y=`z-score`, group=APOE, fill = APOE)) + 
...
# boxplot
figs1.stg.2 = ggplot(data = my.df, aes(x=APOE, y=`z-score`, group=APOE, fill = APOE)) +
  geom_boxplot(outlier.colour="black", outlier.shape=16,
               outlier.size=2, notch=FALSE) +
  scale_fill_manual(values = rev(to_swap)) +
  ylab("Ave z-score")
  
figs1.stg = figs1.stg.1 + figs1.stg.2 +
  plot_annotation(title = 'B', theme = theme(plot.title = element_text(face = "plain", size = 15)))
ggsave("../TablesFigures/FigureS1-violin-STG.pdf", figs1.stg, width = 5, height = 4, dpi = 300)

fig2d = arrangeGrob(figs1.stg.1, figs1.stg.2, nrow=1, top = 
...
pic = wrap_plots(grobs = plist, guides = 'collect', design = layoutplot) +
  plot_annotation(title = 'B', theme = theme(plot.title = element_text(face = "plain", size = 15)))
ggsave("../TablesFigures/FigureS1-box-STG.pdf", pic, width = 12, height =8)
```
